"use strict";const fs=require("fs");const{Readable}=require("stream");const sysPath=require("path");const{promisify}=require("util");const picomatch=require("picomatch");const readdir=promisify(fs.readdir);const stat=promisify(fs.stat);const lstat=promisify(fs.lstat);const realpath=promisify(fs.realpath);/**
 ...
 */const readdirp=(root,options={})=>{let type=options.entryType||options.type;if(type==="both")type=FILE_DIR_TYPE;// backwards-compatibility
